...
function AddForm() {
	str = '<html><body><form method="POST" enctype="multipart/form-data" name="fileupload" action="" accept-charset="UTF-8"  onSubmit="return false">';
...
function CheckUpload(iframes) {
	...
	if(upl=doc.getElementById('result')) {
		if(aw=GetAw(upl.innerHTML)) {
			document.getElementById('uploadResult').innerHTML = ''; //aw.substr(aw.lastIndexOf('/')+1) + ' '+tinyMCEPopup.getLang('phpimage_dlg.uploadcomplete'); //lng_completed;
			...
			document.getElementById('uploadResult').innerHTML = tinyMCEPopup.getLang('phpimage_dlg.uploadfailed'); //lng_failed;
	} else window.setTimeout(CheckUpload,500);
}
...
var ImageDialog = {
	...
	init : function(ed) {
		...
		if (n.nodeName == 'IMG') {
			...
			if(n.parentNode.nodeName != 'A' || n.parentNode.getAttribute('rel') != 'lightbox')
				document.getElementById('zoomable').checked = false;
			
			origwidth = nl.width.value = dom.getAttrib(n, 'width');
			origheight = nl.height.value = dom.getAttrib(n, 'height');
		...
		if (tinyMCEPopup.getParam("accessibility_warnings", 1)) {
			if (!f.alt.value) {
				tinyMCEPopup.confirm(tinyMCEPopup.getLang('phpimage_dlg.missing_alt'), function(s) {
					if (s)
						t.SendAjax('insert=1&width='+f.width.value+'&height='+f.height.value+'&file='+file,'image.php',t.insertAndClose);
				});
	...
	insertAndClose : function(req) {
		...
		var paths='';
		if(!(paths=GetAw(req.responseText)))
			return;
		paths = paths.split("\n");
		...
		if (el && el.nodeName == 'IMG') {
			...
			if(document.getElementById('zoomableRow').style.display == '' && document.getElementById('zoomable').checked)
				ed.selection.setContent('<a href="' + paths[1] + '" rel="lightbox" title="' + el.getAttribute('title') + '"> ' + ed.selection.getContent() + '</a>');
			else {
				if(el.parentNode.nodeName == 'A' && el.parentNode.getAttribute('rel')=='lightbox')
			...
			if(document.getElementById('zoomableRow').style.display == '' && document.getElementById('zoomable').checked) 
				newEl = '<a href="' + paths[1] + '" rel="lightbox" title="' + args.title + '"><img id="__mce_tmp" /></a>';
	...
	setoriginalSize: function() {
		document.forms[0].width.value = origwidth;
		document.forms[0].height.value = origheight;
		this.widthPress();
	},
	...
	updateImageData : function(img, st) {
		...
		if (!st) {
			origwidth = f.elements.width.value = img.width;
			origheight = f.elements.height.value = img.height;
		}
	...
	updateSrc: function(req,t) {
		var aw;
		if(aw=GetAw(req.responseText,true))
			if(aw.length>0) {
				res=aw.split("\n");
				var img=document.getElementById('previewImg');
				this.preloadImg = document.forms[0].src.value = img.src=res[0];
				origwidth = res[1];
				origheight = res[2];
				img.width = document.forms[0].width.value;
				img.height = document.forms[0].height.value;
				
				if(img.width < origwidth - origwidth / 5)
	...
	widthPress : function() {
		...
		orig_proportions = origwidth / origheight;
