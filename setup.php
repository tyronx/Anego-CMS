...
<?php
	...
	$bad = '<img src="styles/default/img/cross.png" alt="not ok"> ';
	$configGood = true;
		$str='PHP Version 5 or higher';
		if(intval(substr(phpversion(),0,1))>=5) echo $good.$str;
			else  { 
		...
		$.get('rwcheck','',function(data) {
			...
			$('#rwcheck').html('<img src="styles/default/img/warning.png" alt="ok"> (apache httpd.conf) mod_rewrite<br>&nbsp;&nbsp;&nbsp;Anego can run without mod_rewrite but you will have to add the following line to the conf.inc.php:<br><div class="box" style="margin-left:10px; width:500px;">$cfg[\'fancyURLs\'] = false;</div>');	});
		</script>
		<?php } else 
...
<?php
	...
	if($configGood) {
		...
		foreach($writeRequired as $f) {
			...
			if(is_dir($f)) $writeable=CheckWriteableRec($f);
			if($writeable)
				echo $good.$f.'<br>';
			else {
				echo $bad.$f;
				if(is_writeable($f) && is_dir($f)) echo ' <span class="err">- you need to allow write access to the folder and all files inside!</span>';
				echo '<br>';
			}
	...
	?>
		...
		<?php
				...
				$icon=$good;
				$err='';
				if(!$sql_link) { $icon=$bad; $err=' <span class="err">- couldn\'t connect to database</span>'; }
				else if(!@mysql_select_db(SQLDB)) { 
					$sql_link=0; $icon=$bad; $err=' <span class="err">- connected but couldn\'t select database (create a database with name: <form style="display:inline;" action="setup.php?a=crdb" method="post"><input type="text" size="10" name="dbname"> <input type="submit" value="Create">)</form></span>'; 
				}
				
				echo $icon.'MySQL Info'.$err.'<br>';
				$err='';
				
				if(file_exists('styles/'.STYLE.'/templates/index.tpl')) echo $good.'Page design';
				else echo $bad.'Page design <span class="err">- \'styles/'.STYLE.'/templates/index.tpl not found.\'</span>';
				echo '<br>';
				$url=parse_url($cfg['domain']);
				
				$path=dirname($_SERVER['REQUEST_URI']).'/';
				if($path[0]=='/') $path = substr($path,1);
				if($path==substr($url['path'],1)) echo $good;
					else { echo $bad; $sql_link=0; $err='<div style="padding-left:20px;">Please add the following line to conf.inc.php:<div class="box">$cfg[\'domain\'].=\''.$path.'\';</div>'; }
				echo 'Anego path'.$err;
		...
		?>
			...
			<?php
					if(isset($_GET['a']) && $_GET['a']=='gu') {
						...
						} else { $users=Array(); $userroles=Array(); }
			...
			?>
					...
					<?php echo (count($user_accounts)>0?$good:$bad).count($user_accounts); ?> Configured user(s)<br>
					</li>
					...
					<?php
						if($tablesOK && count($user_accounts)) {
							...
							echo '<li>All done. <a href="'.$cfg['domain'].$link.'">Enjoy setting up your freshly installed Anego CMS!</a><br>Start by creating your first page in the menu</li>';
						}
