...
switch ($_GET['a']) {
	...
	case 'ges':
		...
		$q = "SELECT module_id, element_id, style, padding, alignment from " . PAGE_ELEMENT . " where page_id=" . intval($_GET['page_id']) . " and element_id=" . intval($_GET["element_id"]);
		$res = mysql_query($q);
	...
	case 'ses':
		$q = "UPDATE " . PAGE_ELEMENT . " SET style='".mysql_real_escape_string($_POST['style'])."', padding='".mysql_real_escape_string($_POST['padding'])."', alignment='".mysql_real_escape_string($_POST['alignment'])."' where page_id=" . intval($_POST['page_id']) . " and element_id=" . intval($_POST["element_id"]);
